"""
hero-auth — Авторизация игроков (регистрация и вход)

За что отвечает:
- POST action=register — создание нового аккаунта (ник, пароль, аватарка, пол)
  Валидирует ник (3-32 символа, уникальность), хеширует пароль SHA256 + соль,
  создаёт записи в таблицах users и heroes
- POST action=login — вход по нику и паролю
  Проверяет хеш пароля, возвращает user_id и аватарку из heroes

Таблицы БД: users (логин/пароль), heroes (игровые данные)
