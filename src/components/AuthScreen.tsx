 ...
 *
 * За что отвечает:
 * - Форма входа (ник + пароль)
 * - Форма регистрации (выбор пола, аватарки, ник, пароль)
 * - 12 аватарок: 6 мужских (Воин, Разбойник, Страж, Лучник, Варвар, Вождь)
 *   и 6 женских (Чародейка, Ведьма, Жрица, Буревестник, Друидка, Провидица)
 * - Отправляет запрос на бэкенд hero-auth
 * - Сохраняет сессию в localStorage
 */
import { useState } from "react";

const HERO_AUTH_URL =
  "https://functions.poehali.dev/181dbb92-6f9b-4920-af86-aeb32b3b83fd";

const MALE_AVATARS = [
  {
    id: "m1",
    emoji:
      "C:\Users\Александр и Юлечка\Desktop\e035c01c-5597-4de9-9035-acf3c75502ce.png",
    label: "Воин",
  },
...
export default function AuthScreen({ onAuth }: AuthScreenProps) {
  ...
  const currentAvatarEmoji =
    [...MALE_AVATARS, ...FEMALE_AVATARS].find((a) => a.id === selectedAvatar)
      ?.emoji ?? "⚔️";
            ...
            <form onSubmit={handleSubmit}>
              {mode === "register" && (
                <div style={{ marginBottom: 16 }}>
                    ...
                    {(["male", "female"] as const).map((g) => (
                      <button
                        ...
                        style={{
                          ...
                          padding: "8px",
                          borderRadius: 4,
                          border: `2px solid ${gender === g ? "var(--crimson)" : "var(--parchment-border)"}`,
                          background: gender === g ? "#fff5f0" : "#faf6e8",
                          cursor: "pointer",
                          fontWeight: 700,
                          fontSize: 13,
                          color:
                  ...
                  >
                    Аватарка — {currentAvatarEmoji}{" "}
                    {
                      [...MALE_AVATARS, ...FEMALE_AVATARS].find(
                    ...
                    {avatars.map((av) => (
                      <button
                        ...
                        style={{
                          padding: "8px 4px",
                          borderRadius: 6,
                          border: `2px solid ${selectedAvatar === av.id ? "var(--crimson)" : "var(--parchment-border)"}`,
                          background:
                            selectedAvatar === av.id ? "#fff5f0" : "#faf6e8",
                          cursor: "pointer",
                          textAlign: "center",
                          fontSize: 22,
                          lineHeight: 1,
